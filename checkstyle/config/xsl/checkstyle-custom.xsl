<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
                ...
                <table border="0" cellpadding="0" cellspacing="0" width="100%">
                    ...
                    <tr>
                        <td class="text-align:right">Designed for use with <a
                                href='https://checkstyle.org/'>CheckStyle</a> and <a
    ...
    <xsl:template match="checkstyle" mode="filelist">
        ...
        <table class="log" border="0" cellpadding="5" cellspacing="2" width="100%">
            ...
            <xsl:for-each select="file[@name and generate-id(.) = generate-id(key('files', @name))]">
                ...
                <xsl:variable
                    name="errorCount" select="count(key('files', @name)/error[@severity='error'])" />
                <xsl:variable
                    name="warningCount"
                    select="count(key('files', @name)/error[@severity='warning'])" />
                <xsl:variable
                    name="infoCount" select="count(key('files', @name)/error[@severity='info'])" />
    ...
    <xsl:template match="file">
        ...
        <table
            class="log" border="0" cellpadding="5" cellspacing="2" width="100%">
            <tr>
                <th>Severity</th>
    ...
    <xsl:template match="checkstyle" mode="summary">
        ...
        <xsl:variable
            name="errorCount" select="count(file/error[@severity='error'])" />
        <xsl:variable
            name="warningCount" select="count(file/error[@severity='warning'])" />
        <xsl:variable
            name="infoCount" select="count(file/error[@severity='info'])" />
        <table class="log"
            border="0" cellpadding="5" cellspacing="2" width="100%">
            <tr>
                <th>Files</th>
                <th>Errors</th>
                <th>Warnings</th>
